...
export default function TransparencyPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      axios.get("https://asaan-ration-d15a.vercel.app/donations"),
      axios.get("https://asaan-ration-d15a.vercel.app/request"),
    ])
      .then(([donRes, reqRes]) => {
  ...
  // Stats
  const totalDonationsThisMonth = donations.filter(
    (d) => d.createdAt && isThisMonth(new Date(d.createdAt))
  ).length;
  ...
  const stats = [
    {
      label:
        language === "ur" ? "اس ماہ کی کل عطیات" : "Total Donations This Month",
  ...
  const feed = [
    ...donations
      ...
      .map((d) => ({
        type: "donation",
        donor:
          d.name || (language === "ur" ? "گمنام" : t("transparency.anonymous")),
        item: `${d.foodType || ""}${
          d.quantity ? ` - ${d.quantity}${d.quantityUnit || ""}` : ""
        }`,
    ...
    ...requests
      ...
      .map((r) => ({
        ...
        time: r.createdAt,
        status: "delivered",
        recipient: `${r.name || (language === "ur" ? "خاندان" : "Family")}${
          r.familySize
            ? ` (${r.familySize} ${language === "ur" ? "افراد" : "members"})`
            : ""
        }`,
      })),
  ]
              ...
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  <li className="flex items-center gap-2">
                    ...
                    {language === "ur"
                      ? "ہر عطیہ کا مکمل ریکارڈ"
                      : "Every donation from source to recipient"}
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-primary rounded-full"></div>
                    {language === "ur"
                      ? "حقیقی وقت میں ترسیل کی صورتحال"
                      : "Real-time delivery status updates"}
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-primary rounded-full"></div>
                    {language === "ur"
                      ? "اثر کے اعداد و شمار اور نتائج"
                      : "Impact metrics and family outcomes"}
                  </li>
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-2 text-sm">
                  <li className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-primary rounded-full"></div>
                    {language === "ur"
                      ? "عطیہ دہندگان کے لیے گمنامی کا آپشن"
                      : "Donor anonymity options available"}
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-primary rounded-full"></div>
                    {language === "ur"
                      ? "وصول کنندگان کی تفصیلات خفیہ رکھی جاتی ہیں"
                      : "Recipient details kept confidential"}
                  </li>
                  <li className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-primary rounded-full"></div>
                    {language === "ur"
                      ? "ڈیٹا کو محفوظ طریقے سے ہینڈل کیا جاتا ہے"
